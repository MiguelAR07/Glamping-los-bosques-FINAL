...
export const CabinCard = ({ 
  ...
  return (
    ...
    >
      ...
      {/* Información del Contenido (Paddings más pequeños y refinados) */}
      <div className="p-4 md:p-5 flex flex-col flex-1 min-w-0">
        <h3 className="text-base md:text-lg font-bold text-stone-900 mb-1.5 tracking-tight line-clamp-1">
          {cabin.nombre}
        </h3>
        
        {showDescription && (
          <p className="text-stone-500 text-xs md:text-sm line-clamp-2 mb-4 leading-relaxed flex-1">
            {cabin.descripcion}
          </p>
        )}
        
        {showFeatures && cabin.features && (
          <div className="mb-4 mt-auto">
            <ul className="grid grid-cols-2 gap-2">
              {cabin.features.slice(0, maxFeatures).map((feature: string, i: number) => (
                <li key={i} className="flex items-center gap-1.5 text-stone-600 text-xs font-medium min-w-0">
        ...
        <div className="mt-auto pt-3 border-t border-stone-100 flex items-center justify-between gap-2">
          <div className="min-w-0">
            <span className="block text-[10px] text-stone-400 font-semibold uppercase tracking-wider">Desde</span>
            <span className="text-base md:text-lg font-bold text-emerald-700 block truncate">
